"use client";

import { useState } from "react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog";
import { ToastAlert } from "@/config/toast";
import { Copy, Check, Mail, Phone, ExternalLink, Share2, FileDown, FileImage } from "lucide-react";
import { jsPDF } from "jspdf";
import { captureElementAsCanvas, downloadAsPng, downloadAsPdf } from "@/lib/capture-invoice";

type ShareQuotationDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  quotationNumber: string;
  clientName: string;
  total: string;
  currency: string;
  shareUrl: string;
  isDraft?: boolean;
  onShare?: () => void;
  quotationElementId?: string;
};

type ShareChannel = {
  id: string;
  label: string;
  icon: React.ReactNode;
  color: string;
  getUrl: (url: string, message: string) => string;
};

const ShareQuotationDialog = ({
  open,
  onOpenChange,
  quotationNumber,
  clientName,
  total,
  currency,
  shareUrl,
  isDraft,
  onShare,
  quotationElementId,
}: ShareQuotationDialogProps) => {
  const markAsSentIfDraft = () => {
    if (isDraft && onShare) onShare();
  };
  const [copied, setCopied] = useState(false);
  const [exporting, setExporting] = useState<"pdf" | "image" | null>(null);

  const message = `Hi ${clientName}, here is your quotation ${quotationNumber} for ${currency} ${total}. You can view it here: ${shareUrl}`;

  const channels: ShareChannel[] = [
    {
      id: "whatsapp",
      label: "WhatsApp",
      icon: (
        <svg viewBox="0 0 24 24" className="size-5 fill-current">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
        </svg>
      ),
      color: "bg-[#25D366] hover:bg-[#1da851] text-white",
      getUrl: (url, msg) => `https://wa.me/?text=${encodeURIComponent(msg)}`,
    },
    {
      id: "email",
      label: "Email",
      icon: <Mail className="size-5" />,
      color: "bg-blue-600 hover:bg-blue-700 text-white",
      getUrl: (url, msg) =>
        `mailto:?subject=${encodeURIComponent(`Quotation ${quotationNumber}`)}&body=${encodeURIComponent(msg)}`,
    },
    {
      id: "sms",
      label: "Text Message",
      icon: <Phone className="size-5" />,
      color: "bg-emerald-600 hover:bg-emerald-700 text-white",
      getUrl: (url, msg) => `sms:?body=${encodeURIComponent(msg)}`,
    },
    {
      id: "telegram",
      label: "Telegram",
      icon: (
        <svg viewBox="0 0 24 24" className="size-5 fill-current">
          <path d="M11.944 0A12 12 0 000 12a12 12 0 0012 12 12 12 0 0012-12A12 12 0 0012 0a12 12 0 00-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 01.171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
        </svg>
      ),
      color: "bg-[#0088cc] hover:bg-[#006da3] text-white",
      getUrl: (url, msg) =>
        `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(`Quotation ${quotationNumber} for ${currency} ${total}`)}`,
    },
    {
      id: "x",
      label: "X (Twitter)",
      icon: (
        <svg viewBox="0 0 24 24" className="size-5 fill-current">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
        </svg>
      ),
      color:
        "bg-black hover:bg-gray-800 text-white dark:bg-white dark:text-black dark:hover:bg-gray-200",
      getUrl: (url, msg) =>
        `https://twitter.com/intent/tweet?text=${encodeURIComponent(`Quotation ${quotationNumber} for ${currency} ${total}`)}&url=${encodeURIComponent(url)}`,
    },
    {
      id: "facebook",
      label: "Facebook",
      icon: (
        <svg viewBox="0 0 24 24" className="size-5 fill-current">
          <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
        </svg>
      ),
      color: "bg-[#1877F2] hover:bg-[#0d65d9] text-white",
      getUrl: (url) => `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`,
    },
  ];

  const handleCopy = async () => {
    await navigator.clipboard.writeText(shareUrl);
    setCopied(true);
    ToastAlert.success("Link copied to clipboard");
    setTimeout(() => setCopied(false), 2000);
    markAsSentIfDraft();
  };

  const handleShare = (channel: ShareChannel) => {
    const url = channel.getUrl(shareUrl, message);
    window.open(url, "_blank", "noopener,noreferrer");
    markAsSentIfDraft();
  };

  const captureQuotationCanvas = async () => {
    if (!quotationElementId || typeof document === "undefined") return null;
    const el = document.getElementById(quotationElementId);
    if (!el) return null;
    return captureElementAsCanvas(el);
  };

  const handleExportImage = async () => {
    markAsSentIfDraft();
    setExporting("image");
    try {
      const canvas = await captureQuotationCanvas();
      if (!canvas) {
        ToastAlert.error("Could not capture quotation");
        return;
      }
      downloadAsPng(canvas, quotationNumber);
    } catch {
      ToastAlert.error("Export failed");
    } finally {
      setExporting(null);
    }
  };

  const handleExportPdf = async () => {
    markAsSentIfDraft();
    setExporting("pdf");
    try {
      const canvas = await captureQuotationCanvas();
      if (!canvas) {
        ToastAlert.error("Could not capture quotation");
        return;
      }
      downloadAsPdf(canvas, quotationNumber, jsPDF);
    } catch {
      ToastAlert.error("Export failed");
    } finally {
      setExporting(null);
    }
  };

  const handleNativeShare = async () => {
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Quotation ${quotationNumber}`,
          text: message,
          url: shareUrl,
        });
        markAsSentIfDraft();
      } catch {
        // user cancelled
      }
    }
  };

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Share Quotation</DialogTitle>
          <DialogDescription>
            Share quotation <span className="font-medium">{quotationNumber}</span> with {clientName}{" "}
            via any channel.
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4 py-2">
          {quotationElementId && (
            <div className="flex gap-2">
              <Button
                variant="outline"
                size="sm"
                className="flex-1"
                onClick={handleExportImage}
                disabled={!!exporting}
              >
                <FileImage className="mr-2 size-4" />
                {exporting === "image" ? "Exporting..." : "Export as Image"}
              </Button>
              <Button
                variant="outline"
                size="sm"
                className="flex-1"
                onClick={handleExportPdf}
                disabled={!!exporting}
              >
                <FileDown className="mr-2 size-4" />
                {exporting === "pdf" ? "Exporting..." : "Export as PDF"}
              </Button>
            </div>
          )}

          <div className="space-y-1.5">
            <p className="text-sm font-medium">Quotation Link</p>
            <div className="flex gap-2">
              <Input
                readOnly
                value={shareUrl}
                className="bg-muted text-xs"
                onClick={(e) => (e.target as HTMLInputElement).select()}
              />
              <Button variant="outline" size="icon" className="shrink-0" onClick={handleCopy}>
                {copied ? (
                  <Check className="size-4 text-emerald-600" />
                ) : (
                  <Copy className="size-4" />
                )}
              </Button>
            </div>
          </div>

          <div className="space-y-1.5">
            <p className="text-sm font-medium">Share via</p>
            <div className="grid grid-cols-3 gap-2">
              {channels.map((channel) => (
                <button
                  key={channel.id}
                  type="button"
                  onClick={() => handleShare(channel)}
                  className={`flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${channel.color}`}
                >
                  {channel.icon}
                  <span className="truncate">{channel.label}</span>
                </button>
              ))}
            </div>
          </div>

          {typeof navigator !== "undefined" && "share" in navigator && (
            <Button variant="outline" className="w-full" onClick={handleNativeShare}>
              <Share2 className="mr-2 size-4" />
              More sharing options...
            </Button>
          )}

          <Button variant="outline" className="w-full" asChild>
            <a href={shareUrl} target="_blank" rel="noopener noreferrer">
              <ExternalLink className="mr-2 size-4" />
              Open Public Quotation Page
            </a>
          </Button>
        </div>
      </DialogContent>
    </Dialog>
  );
};

export { ShareQuotationDialog };
